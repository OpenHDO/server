      ...
      <header className="flex flex-wrap items-start justify-between gap-4 p-4 min-[390px]:p-5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-neutral-500">{linker.id} · v{linker.version}</p>
          </div>
        </div>
...
function StatusMessage({ icon, message }: { icon: ReactNode; message: string }) {
  return <div className="flex items-center gap-2 border-y border-neutral-800 py-4 text-sm text-neutral-500">{icon}{message}</div>;
}
